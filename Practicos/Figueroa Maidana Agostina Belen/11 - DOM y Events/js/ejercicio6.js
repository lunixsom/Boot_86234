...
const botonAcordeonHTML = document.querySelectorAll(".acordeon-btn");
const contenidoAcordeonHTML = document.querySelectorAll(".acordeon-contenido");

botonAcordeonHTML.forEach(element => {

    element.addEventListener("click", () => {

        const estabaAbierto = element.classList.contains("abierto");

        botonAcordeonHTML.forEach(boton => {
            boton.classList.remove("abierto");
        });

        contenidoAcordeonHTML.forEach(contenido => {
            contenido.classList.remove("abierto");
        });

        if (!estabaAbierto) {
            element.classList.add("abierto");
            element.nextElementSibling.classList.add("abierto");
        }
