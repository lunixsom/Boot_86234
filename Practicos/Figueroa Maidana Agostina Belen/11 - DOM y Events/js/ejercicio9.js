...
const ulListaContactoHTML = document.querySelector("#lista-contactos");
const contadorHTML = document.querySelector("#contador");
const botonAgregarHTML = document.querySelector("#btn-agregar");
const inputNombreHTML = document.querySelector("#input-nombre");
const inputEmailHTML = document.querySelector("#input-email");
const buscadorHTML = document.querySelector("#buscador");

function renderContactos(lista) {

  ulListaContactoHTML.innerHTML = "";

  lista.forEach(contactos => {

    const li = document.createElement("li");
    li.innerHTML = `<strong>Nombre:</strong> ${contactos.nombre} - <strong>Email:</strong> ${contactos.email} - <button class="btn-eliminar" data-id="${contactos.id}">✕</button> <hr>`;
    ulListaContactoHTML.appendChild(li);
...
contadorHTML.textContent = `${contactos.length} contactos`;

botonAgregarHTML.addEventListener("click", () => {

  if (inputNombreHTML.value === "" || inputEmailHTML.value === "") {
    return;
  }

  const nuevoContacto = {
    id: Date.now(),
    nombre: inputNombreHTML.value,
    email: inputEmailHTML.value
  };

  contactos.push(nuevoContacto);

  inputNombreHTML.value = "";
  inputEmailHTML.value = "";
...
ulListaContactoHTML.addEventListener("click", (evento) => {
...
buscadorHTML.addEventListener("input", (evento) => {

    const valor = evento.target.value.toLowerCase();

    const contactosFiltrados = contactos.filter(contacto =>
        contacto.nombre.toLowerCase().includes(valor)
    );

    renderContactos(contactosFiltrados);

    if (contactosFiltrados.length === 0) {
        contadorHTML.textContent = "No se encontraron contactos";
    } else {
        contadorHTML.textContent = `${contactosFiltrados.length} contactos`;
    }
