...
document.querySelector("#titulo").textContent = "Equipo de trabajo";
document.querySelector("#descripcion").innerHTML = "Conocé a nuestro equipo. <strong>Somos 3 integrantes.</strong>"

document.querySelector("#tarjeta-1").classList.add("destacado");

document.querySelector("#tarjeta-3").classList.add("inactivo");

document.querySelectorAll(".nombre").forEach((elemento) =>{
elemento.textContent = elemento.textContent.toUpperCase();
});

document.querySelector("#tarjeta-2").classList.toggle("oculto");
document.querySelector("#tarjeta-2").classList.toggle("oculto");


//Al utilizar toggle por primera vez se agrega la clase "oculto" y la tarjeta desaparece.
//Al utilizarlo por segunda vez quita la clase "oculto" y la tarjeta vuelve a aparecer.