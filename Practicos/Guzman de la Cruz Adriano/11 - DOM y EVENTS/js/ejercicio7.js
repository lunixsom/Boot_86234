...
function renderLista(array){
  ...
  if(array.length === 0){
    contador.textContent = "No se encontraron paises";
  } else{
    contador.textContent = `${array.length} paises`;
  }
}
...
buscador.addEventListener("input",function(){
  ...
  const resultados = paises.filter(function(pais){
    return pais.toLowerCase().includes(textoBuscado);
  });
