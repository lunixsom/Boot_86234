...
contarOcurrencias = (nums, element) => {
  return nums.filter(el => el === element).length;
}

letrasNum = (text, letra) => {
  return text.split('').filter(char => char === letra).length;
}
...
console.log('Ocurrencias de "e":', letrasNum("Hola, ¿cómo estás? Espero que estés bien.", "e"));