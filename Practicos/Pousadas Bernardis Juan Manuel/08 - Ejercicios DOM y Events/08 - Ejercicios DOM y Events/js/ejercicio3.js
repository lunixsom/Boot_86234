...
const productosElectronica = productosArray.filter((producto) => {
  return producto.dataset.categoria === "electronica";
});
const resultado = document.querySelector("#resultado");
resultado.textContent = `Hay ${productosElectronica.length} productos de electrónica`;

let productoCaro = productosArray[0];
productosArray.forEach((producto) => {
  if (Number(producto.dataset.precio) > Number(productoCaro.dataset.precio)) {
    productoCaro = producto;
  }
});
productoCaro.classList.add("destacado");
