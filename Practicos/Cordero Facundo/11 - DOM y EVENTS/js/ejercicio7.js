...
function renderLista(arrayDePaises) {
  listaUl.innerHTML = '';

  arrayDePaises.forEach((pais) => {
    const li = document.createElement('li');
    li.textContent = pais;
    listaUl.appendChild(li);
  });

  if (arrayDePaises.length === 0) {
    contadorP.textContent = "No se encontraron países";
  } else {
    contadorP.textContent = `${arrayDePaises.length} países`;
  }
}
