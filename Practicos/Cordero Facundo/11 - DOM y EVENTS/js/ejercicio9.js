...
function renderContactos(lista) {
  ...
  if (lista.length === 0) {
    contadorP.textContent = "No se encontraron contactos";
  } else {
    contadorP.textContent = `${lista.length} ${lista.length === 1 ? 'contacto' : 'contactos'}`;
  }
}
...
listaUl.addEventListener('click', function (e) {
  ...
  if (btnEliminar) {
    const idAEliminar = Number(btnEliminar.dataset.id);

    contactos = contactos.filter((contacto) => contacto.id !== idAEliminar);
