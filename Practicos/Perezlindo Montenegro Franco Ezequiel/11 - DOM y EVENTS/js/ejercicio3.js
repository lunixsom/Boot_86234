...
productos.forEach((producto) => {
    console.log(
        `id: ${producto.dataset.id} | categoría: ${producto.dataset.categoria} | precio: ${producto.dataset.precio}`
    );
});
...
let masCaro = productosArray[0];

productosArray.forEach((producto) => {
    const precioActual = Number(producto.dataset.precio);
    const precioMasCaro = Number(masCaro.dataset.precio);
