...
const productosElectronica = productosArray.filter((producto) => producto.dataset.categoria === 'electronica');
const resultadoDiv = document.getElementById('resultado');
resultadoDiv.textContent = `Hay ${productosElectronica.length} productos de electronica`;

let productoMasCaro = productosArray[0];
productosArray.forEach((producto) => {
    const precio = Number(producto.dataset.precio);
    if (precio > Number(productoMasCaro.dataset.precio)) {
