...
const tarjeta1HTML = document.getElementById("tarjeta-1");
tarjeta1HTML.classList.add("destacado");

const tarjeta3HTML = document.getElementById("tarjeta-3");
tarjeta3HTML.classList.add("inactivo");

const claseNombreHTML = document.querySelectorAll(".nombre");
console.log(claseNombreHTML);

claseNombreHTML.forEach((elemento) =>{
    elemento.textContent = elemento.textContent.toUpperCase();
})

const tarjeta2HTML = document.getElementById("tarjeta-2");
//Se puede visualizar con toggle() como agrega la clase si no encuentra y si la encuentra la saca
tarjeta2HTML.classList.toggle("inactivo");
