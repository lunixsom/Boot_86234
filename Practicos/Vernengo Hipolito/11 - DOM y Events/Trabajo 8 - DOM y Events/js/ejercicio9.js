...
const contactosHTML = document.getElementById("lista-contactos");
const contadorHTML = document.getElementById("contador");
const buscadorHTML = document.getElementById("buscador");
const botonAgregarHTML = document.getElementById("btn-agregar");
const inputNombreHTML = document.getElementById("input-nombre");
const inputEmailHTML = document.getElementById("input-email");
const botonEliminarHTML = document.getElementById("btn-eliminar");
...
botonAgregarHTML.addEventListener("click", () =>{
  const nombre = inputNombreHTML.value.trim();
  const email = inputEmailHTML.value.trim();
...
buscadorHTML.addEventListener("input", (event) =>{
  ...
  const datos = event.target.value.toLowerCase();

  const contactosEncontrados = contactos.filter((contacto) =>{
    if(contacto.nombre.toLowerCase().includes(datos) || contacto.email.toLowerCase().includes(datos)){
      return true;
    }
  })

  renderContactos(contactosEncontrados);
})
