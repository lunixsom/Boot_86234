...
productos.forEach(producto => {
    ...
    console.log(`id: ${id} | categoria: ${categoria} | precio: ${precio}`);
}); // 1

const arrayProductos = Array.from(productos);
const electronicos = arrayProductos.filter(producto => producto.dataset.categoria === 'electronica');

const resultado = document.querySelector('#resultado');
resultado.textContent = `Hay ${electronicos.length} productos de electrónica`; // 2

let masCaro = arrayProductos[0];

arrayProductos.forEach(producto => {
    if (Number(producto.dataset.precio) > Number(masCaro.dataset.precio)) {
        masCaro = producto;
    }
});

masCaro.classList.add('destacado'); // 3